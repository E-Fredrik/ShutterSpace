...
struct RecentChatRowView: View {
    ...
    var body: some View {
        HStack(spacing: 16.0) {
            ...
            if chat.partnerImageUrl.isEmpty
                || !chat.partnerImageUrl.starts(with: "http")
            {
            ...
            VStack(alignment: .leading, spacing: 4.0) {
                ...
                // Use the boolean flag for UI logic
                Text(
                    chat.isBlocked
                        ? "Message blocked by Admin" : chat.lastMessage
                )
                .font(.subheadline)
                .foregroundColor(chat.isBlocked ? .red : .secondary)
                .italic(chat.isBlocked)
                .lineLimit(2)
            }
        }
